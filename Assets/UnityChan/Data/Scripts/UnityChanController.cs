	...
	private struct GroundCollision
		...
		private void OnTriggerEnter(Collider other)
		{
			if (other.gameObject.tag == "EnemyTag")
			{
				...
				//ゲーム終了
				isEnd = true;
				//stateTextにGANE CLEARを表示（追加）
				this.GameText.GetComponent<Text>().text = "GAME OVER";
				//stateTextにtimeを表示（追加）
				this.stateText.GetComponent<Text>().text = "タイム" + Time.time.ToString("f2") + "秒";
			...
			if (other.gameObject.tag == "GoalTag")
			{
				...
				//stateTextにGANE CLEARを表示（追加）
				this.GameText.GetComponent<Text>().text = "CLEAR!!";
		...
		private void Start()
		{
			...
			this.GameText = GameObject.Find("GameResultText");

			//シーン中のstateTextオブジェクトを取得（追加）
			this.stateText = GameObject.Find("TimeAttack");
